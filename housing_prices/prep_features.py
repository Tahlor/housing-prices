...
def my_assert(df, column="MSZoning"):
    try:
...
def main(training_df, test_df, use_log=True, variable_combinations=[]):
    ...
    my_assert(training_df, "TotalSQF")
    ...
    my_assert(training_df, "TotalSQF")
    my_assert(training_df, "MSZoning_FV")
...
def smart_impute(features, features_to_impute = []):
    ...
    Args:
        features (DataFrame): The dataframe containing data to impute.
        features_to_impute (list): List of string features to impute. If empty, all floats are imputed.
    Returns:
        DataFrame: All features, with newly recoded features
    """
...
def transform_features(features, scaler_normal = None, scaler_01 = None):
    ...
    ## Make huge interaction thing
    if False:
        # bools = main_var_df.select_dtypes(include=["bool", "category"]).columns
        # main_var_df[bools] = main_var_df[bools].astype('int')
        #
        # log_features = main_var_df[prep_features.get_features(main_var_df, "_log")]
        # unlog_features = main_var_df[prep_features.get_features(main_var_df, "_log", inverse=True)]
        #
        # # Create interactions
        # additive, _ = transform(log_features, rename=True, replace=True, trans_type="add")
        # subtractive, _ = transform(log_features, rename=True, replace=True, trans_type="subtract")
        # multiplicative, _ = transform(unlog_features, rename=True, replace=True, trans_type="multiply")
        # divisive, _ = transform(unlog_features, rename=True, replace=True, trans_type="divide")
        pass
